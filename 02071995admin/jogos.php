...
<body>
                        ...
                        <div class="card">
                            <div class="card-header d-flex justify-content-between align-items-center flex-wrap gap-2">
                                ...
                                <div class="d-flex gap-2 flex-wrap">
                                    <button class="btn btn-danger" onclick="confirmRemoveAll()">
                                        <i class="fas fa-trash-alt"></i> Remover Todos os Jogos
                                    </button>
                                    <button class="btn btn-success" onclick="importMaxAPIGames()" id="btnImport">
                                        <i class="fas fa-download"></i> Importar Jogos MaxAPIGames
                                    </button>
    ...
    <script>
        ...
        function confirmRemoveAll() {
            if (confirm('ATENÇÃO: Isso vai remover TODOS os jogos do sistema. Deseja continuar?')) {
                if (confirm('Tem certeza? Esta ação não pode ser desfeita!')) {
                    fetch('ajax/remove_all_games.php', {
                        method: 'POST',
                    ...
                    .catch(error => {
                        console.error('Erro:', error);
                        showToast('error', 'Erro ao remover jogos.');
                    });
                }
            }
        }
        ...
        function importMaxAPIGames() {
            ...
            fetch('ajax/import_maxapigames.php', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' }
            })
            .then(response => response.json())
            .then(data => {
                btn.disabled = false;
                btn.innerHTML = '<i class="fas fa-download"></i> Importar Jogos MaxAPIGames';
                if (data.success) {
                    showToast('success', data.message);
                    setTimeout(() => location.reload(), 1500);
                } else {
