...
#=====================================================#
# DATA CONFIG
function data_maxapigames()
{
	global $mysqli;
	$qry = "SELECT * FROM maxapigames WHERE id=1";
	$res = mysqli_query($mysqli, $qry);
	$data = mysqli_fetch_assoc($res);
	return $data;
}
$data_maxapigames = data_maxapigames();
...
function pegarLinkJogoigamewin($provider, $game, $email)
{
    ...
    if (isset($data['launch_url'])) {
        $games = array('gameURL' => $data['launch_url'], 'error' => null);
    } else {
        $games = array('gameURL' => null, 'error' => $data['msg'] ?? 'Erro desconhecido');
    }
...
/**
 * Função para lançar jogos da MaxAPIGames
 */
function pegarLinkJogoMaxAPIGames($provedor, $game, $email, $saldo)
{
    global $data_maxapigames;
    $keys = $data_maxapigames;
    ...
    // Log para debug
    error_log("[MaxAPIGames] Request: " . $json_data);

    $response = enviarRequest($keys['url'], $json_data);
    $data_response = json_decode($response, true);

    error_log("[MaxAPIGames] Response: " . $response);

    if (isset($data_response['launch_url'])) {
        $games = array('gameURL' => $data_response['launch_url']);
    } else {
        error_log("[MaxAPIGames] Erro - Resposta inválida");
...
function obterWebhooksAtivos($idPreferido, $nomePreferido)
{
    global $mysqli;
    $rows = [];
    $stmt = $mysqli->prepare("SELECT * FROM webhook WHERE status = 1 AND id = ?");
    if ($stmt) {
    ...
    if (count($rows) === 0 && !empty($nomePreferido)) {
        $stmt = $mysqli->prepare("SELECT * FROM webhook WHERE status = 1 AND nome = ?");
        if ($stmt) {
    ...
    if (count($rows) === 0) {
        $res = mysqli_query($mysqli, "SELECT * FROM webhook WHERE status = 1 ORDER BY id ASC LIMIT 1");
        if ($res && mysqli_num_rows($res) > 0) {
